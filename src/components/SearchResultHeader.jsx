...
const SearchResultHeader = () => {
  ...
  return (
    <div className="p-[15px] pb-0 md:pr-5 md:pl-20 md:pt-7 border-b border-[#ebebeb] flex md:block flex-col items-center sticky top-0 bg-white   ">
      ...
      <div className="flex mt-3 ml-[-12px ] ">
        {menu.map((menu, index) => (
          <span
            key={index}
            className={` items-center flex p-3 text-[#5f6368] cursor-pointer relative ${
          ...
          >
            ...
            {selectedMenu === menu.name && (
              <span className="bottom-0 h-[3px] w-[calc(100%-20px)] absolute left-[10px] bg-[#1a73e8] " />
            )}
          </span>
        ))}
      </div>
